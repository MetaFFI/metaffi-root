...
def __getConanPackages(env: Environment, conanfile:str, conandeps:str):
	...
	symbol = '>' if env['PLATFORM'] == 'win32' else '$'
	terminal_curpath = env.Dir('.').abspath+symbol

	conan_install_cmd = f'conan install {conanfile} --build=missing'
	print(f'{terminal_curpath} {conan_install_cmd}')
	envvars = env['ENV'].copy()

	process = subprocess.run(f'{conan_install_cmd}', shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, env=envvars)
	...
	batch_files_after = glob.glob(os.path.join(env.Dir('.').abspath, '*conan*.bat'))
	batch_files_after.extend(glob.glob(os.path.join(env.Dir('.').abspath, '*conan*.sh')))
	...
	# if conandeps is different than SConsDeps Sconscript_conandeps, rename Sconscript_conandeps to conandeps variable
	if conandeps != 'SConscript_conandeps':
		os.rename(env.Dir('.').File('SConscript_conandeps').abspath, env.Dir('.').File(conandeps).abspath)


def LoadConanPackagesInfo(env: Environment, conanfile: str = 'conanfile.txt', conandeps: str = 'SConscript_conandeps') -> None:
	...
	if not env.Dir('.').File(conanfile).exists():
		expected_file_location = env.Dir('.').File(conanfile).abspath
	...
	if not os.path.exists(env.Dir('.').File(conandeps).abspath):
		__getConanPackages(env, conanfile, conandeps)

	conan_info = SCons.Script.SConscript(conandeps)

	# make sure conan_info is not obsolete or from another platform...
	need_to_reinstall_conan_packages = False
	for path in conan_info['conandeps']['CPPPATH']:
		if not os.path.exists(path):
			need_to_reinstall_conan_packages = True
			break
...
def CPPSharedLibrary(env: Environment, target: str, project_name: str, sources: List[str], include_dirs: List[str]|None = None, lib_dirs: List[str]|None = None, libs: List[str]|None = None) -> SCons.Node.NodeList:
	...
	if env['PLATFORM'] != 'win32':
		...
		return env.SharedLibrary(dynamic_lib_full_name, source=sources, CPPPATH=env['CPPPATH'])
	else:
...
def CPPProgram(env: Environment, target: str, project_name: str, sources: List[str], include_dirs: List[str] = [], lib_dirs: List[str] = [], libs: List[str] = []) -> SCons.Node.NodeList:
	...
	if lib_dirs is not None and len(lib_dirs) > 0:
		options.set_lib_directories(lib_dirs)
...
def DynamicLibraryExtension(env: Environment) -> str:
	if env['PLATFORM'] == 'win32':
		...
		if sys.platform == 'linux':
			...
			print(f'Unknown sys.platform: {sys.platform} for environment platform POSIX. Exiting...', file=sys.stderr)
			sys.exit(1)
	else:
...
def IsLinux(env: Environment) -> bool:
	return env['PLATFORM'] == 'posix' and sys.platform == 'linux'

def IsMac(env: Environment) -> bool:
	return env['PLATFORM'] == 'posix' and sys.platform == 'darwin'
...
def SearchConanBinaryFile(env: Environment, package_name: str, binary_name: str) -> str:
	binpaths = []
	if platform.system() == 'Windows':
		for binlib in env['conan_info'][package_name]['BINPATH']:
			binpaths.extend(glob.glob(os.path.join(binlib, binary_name)))
	else:
		for binlib in env['conan_info'][package_name]['LIBPATH']:
			binpaths.extend(glob.glob(os.path.join(binlib, binary_name)))

		# remove symbolic links from list
		binpaths = [binpath for binpath in binpaths if not os.path.islink(binpath) and not os.path.isdir(binpath)]
 
	if len(binpaths) == 0:
		print(f'Binary file {binary_name} not found in {binpaths}. Exiting...', file=sys.stderr)
		sys.exit(1)
 
	if len(binpaths) > 1:
		print(f'Multiple binary files found for {binary_name} in {package_name} ({binpaths}).\nImprove the binary name pattern.\nExiting...', file=sys.stderr)
		sys.exit(1)
...
def GoBuildProgram(target, source, env):
	...
	exit_code = env.Execute(f"go get", chdir=os.path.dirname(source[0]))
	if exit_code:
		print(f"Failed go get command: exit code {exit_code}", file=sys.stderr)
		sys.exit(1)
	
	exit_code = env.Execute(f"go build -o {target} .", chdir=os.path.dirname(source[0]))
	if exit_code:
		print(f"Failed to build Go program: exit code {exit_code}", file=sys.stderr)
		sys.exit(1)
...
def __set_go_env(env: Environment, env_key: str):
	if env_key not in env['ENV']:
		env_val = subprocess.run(['go', 'env', f'{env_key}'], stdout=subprocess.PIPE, check=True).stdout.decode().strip()
		env['ENV'][f'{env_key}'] = env_val
...
def GoBuildCSharedLibBuilder(target, source, env):
	...
	try:
		...
		print(f'{source}{symbol} go get')
		exit_code = env.Execute(f"go get")
		if exit_code:
			print(f'Failed go get command with exit code {exitcode}. Exiting...', file=sys.stderr)
			sys.exit(1)
		
		print(f'{source}{symbol} go build -buildmode=c-shared -gcflags=-shared -o {target} .')
		exit_code = env.Execute(f"go build -buildmode=c-shared -gcflags=-shared -o {target} .")
		if exit_code:
			print(f"Failed building Go dynamic library: exit code {exit_code}", file=sys.stderr)
			sys.exit(1)
	finally:
		os.chdir(curpath)
...
def MetaFFICompileGuest(env: Environment, output_dir: str, source_idl: str) -> SCons.Node.NodeList:
	...
	def compile_metaffi_guest(target, source, env):
		...
		exit_code = env.Execute(f"metaffi -c --idl {source_idl} -o {output_dir} -g")
		if exit_code:
			print(f"Failed to compile MetaFFI guest with exit code {exit_code}", file=sys.stderr)
			sys.exit(1)
...
def MetaFFICompileHost(target, source, env):
	...
	exit_code = env.Execute(f'metaffi -c --idl \'{source}\' -o \'{target}\' -h {plugin}')
	if exit_code:
...
def JavaJarBuilder(target, source, env):
	...
	exit_code = env.Execute(' '.join(javac_cmd))
	if exit_code:
		print(f"Failed to compile Java with exit code {exit_code}", file=sys.stderr)
		sys.exit(1)
	
	# create jar file
	jar_cmd = ['jar', 'cvf', jar_file, '-C', class_files_dir, '.']
	exit_code = env.Execute(' '.join(jar_cmd))
	if exit_code:
		print(f"Failed to pack a Jar file with exit code {exit_code}", file=sys.stderr)
		sys.exit(1)
